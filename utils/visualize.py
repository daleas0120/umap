...
def display_gt(image, gt_boxes, name, figsize=(4, 4), colors = None, titles=None, cols=4, cmap=None, norm=None,
    ...
    for jdx in range(len(gt_boxes)):
        y1_gt = gt_boxes[jdx][0]
        x1_gt = gt_boxes[jdx][1]
        y2_gt = gt_boxes[jdx][2]
        x2_gt = gt_boxes[jdx][3]

        p_gt = patches.Rectangle((x1_gt, y1_gt), x2_gt - x1_gt, y2_gt - y1_gt, linewidth=2,
...
def display_instances(results_name, image, boxes, masks, class_ids, class_names, gt_boxes, gt_classes,
                      scores=None, save_img=True, title="",
                      figsize=(10, 10), ax=None, show_mask=True,
                      show_bbox=True,
    ...
    color_by_class = []
    color_by_class.append((0.0, 0.0, 0.0))
    color_by_class.append((0.0, 0.0, 1.0))
    color_by_class.append((0.0, 1.0, 0.0))
    color_by_class.append((0.0, 1.0, 1.0))
    color_by_class.append((1.0, 0.0, 0.0))
    color_by_class.append((1.0, 0.0, 1.0))
    color_by_class.append((1.0, 1.0, 0.0))
    color_by_class.append((1.0, 1.0, 1.0))
    ...
    if show_bbox:
        for jdx in range(len(gt_boxes)):
            ...
            p_gt = patches.Rectangle((x1_gt, y1_gt), x2_gt - x1_gt, y2_gt - y1_gt, linewidth=1,
                                alpha=0.7, linestyle="dashed",
                                edgecolor=color_by_class[gt_classes[jdx]], facecolor='none')
            ax.add_patch(p_gt)

    for i in range(N):
        if class_ids[i] < len(color_by_class):
            color = color_by_class[class_ids[i]]
        else:
            color = colors[i]
